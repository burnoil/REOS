...
# Place the Windows taskbar in auto-hide mode without terminating Explorer.
$abd = New-Object REOSNative+APPBARDATA
$abd.cbSize = [Runtime.InteropServices.Marshal]::SizeOf($abd)
$abd.lParam = [IntPtr]3 # ABS_AUTOHIDE | ABS_ALWAYSONTOP
[void][REOSNative]::SHAppBarMessage(0x0000000A, [ref]$abd) # ABM_SETSTATE

# Hide desktop icons only when they are currently visible.
$advanced = 'HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced'
Set-ItemProperty -Path $advanced -Name HideIcons -Type DWord -Value 1
$desktopView = Get-DesktopView
if ($desktopView -ne [IntPtr]::Zero -and [REOSNative]::IsWindowVisible($desktopView)) {
